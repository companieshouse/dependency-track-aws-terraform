resource "aws_ecs_service" "ecs-service" {
  ...
  load_balancer {
    ...
    container_name   = var.use_eric_reverse_proxy ? "eric" : local.target_container
  }
...
resource "aws_ecs_task_definition" "ecs-task-definition" {
  ...
  cpu                      = var.use_eric_reverse_proxy ? var.required_cpus + var.eric_cpus : var.service_cpu > 0 ? var.service_cpu : var.required_cpus
  memory                   = var.use_eric_reverse_proxy ? var.required_memory + var.eric_memory : var.service_memory > 0 ? var.service_memory : var.required_memory
